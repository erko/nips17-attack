...
from slim.nets import inception, resnet_v2, vgg, nets_factory
...
def main(_):
    ...
    with tf.Graph().as_default():
        ...
        eval_models = ['inception_resnet_v2']
        for model_name in eval_models:
            ...
            logits = model(model_name, x_input, num_classes, True, label_offset=label_offset, scope=scope)
            ...
            models_probabilities.append(prob)

        y = tf.get_variable('y', [batch_size, num_classes], tf.float32, tf.constant_initializer())
        probabilities = tf.reduce_mean(tf.stack(models_probabilities), axis=0)
        y_assign = tf.assign(y, tf.one_hot(tf.argmax(probabilities, axis=1), num_classes))
        ...
        for i, model_name in enumerate(models_scopes.keys()):
            ...
            reuse = model_name in eval_models

            model_name = model_name if model_name not in network_mapping else network_mapping[model_name]

            logits = model(model_name, clipped_x, num_classes, True,
                           label_offset=label_offset, scope=scope, reuse=reuse)
        ...
        with tf.Session() as sess:
            ...
            for filenames, images in load_images(FLAGS.input_dir, batch_shape):
                ...
                _ = sess.run(y_assign, feed_dict={x_input: images})
